                  ...
                  : proxies.map(p => (
                    <div key={p.id} className="flex items-center justify-between bg-surface2 rounded-lg px-3 py-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button className="text-muted hover:text-danger" onClick={async () => { await fetch(`/api/proxies/${p.id}`,{method:'DELETE'}); load() }}><Trash2 size={13}/></button>
                      </div>
                    </div>
                  ))
                }
            ...
            <div className="card p-4">
              ...
              {[['Авто-ротация',true],['Рандомные задержки',true],['Авто-замена при бане',true]].map(([l,v]) => (
                <div key={l} className="flex items-center justify-between mb-2">
                  <span className="text-xs text-muted">{l}</span><Toggle checked={v} onChange={()=>{}}/>
                </div>
              ))}
            </div>
          </div>
